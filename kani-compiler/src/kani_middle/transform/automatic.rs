...
impl TransformPass for AutomaticHarnessPass {
    ...
    fn transform(&mut self, tcx: TyCtxt, body: Body, instance: Instance) -> (bool, Body) {
        ...
        for local_decl in fn_to_verify_body.arg_locals().iter() {
            ...
            .unwrap();
            // let fn_name = fn_to_verify.name();
            // let kani_any_name = kani_any_inst.name();
            // let kani_any_body = kani_any_inst.body().unwrap();

            // let term = &kani_any_body.blocks[0].terminator;
            // let real_kani_any = if let TerminatorKind::Call { func, .. } = &term.kind {
            //     if let Operand::Constant(op) = func {
            //         let kind = op.const_.ty().kind();
            //         let (def, args) = kind.fn_def().unwrap();
            //         let real_kani_any = Instance::resolve(def, &args).unwrap();
            //         let real_kani_any_body = real_kani_any.body().unwrap();
            //         // dbg!(&real_kani_any_body);
            //         real_kani_any
            //     } else {
            //         kani_any_inst
            //     }
            ...
            //dbg!(&real_kani_any.name(), &real_kani_any.body().unwrap());

            // dbg!(&fn_name, &kani_any_name, &kani_any_body);
            harness_body.insert_call(
                &kani_any_inst,
                &mut source,
                InsertPosition::Before,
